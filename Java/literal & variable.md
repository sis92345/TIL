...
- 변수는 <u>변수의 선언 -> 변수의 초기화</u> 과정을 거친다.
  ...
  - 변수의 자료형을 선언하는 이유
    - **변수에 저장되는 값의 형태를 결정한다.**
      - int형에는 정수만 들어가며, double에는 실수만 들어간다.
      - 이 개념은 참조형에서도 마찬가지이다. 참조형은 인스턴스의 주소를 저장하는 공간으로 4byte의 크기이다. 객체를 선언할때 일일히 Car car = new Car(), Driver AN = new Driver("1종보통")과 같이 일일히 클래스의 이름으로일일히 자료형을 선언한다. 그 이유가 바로 변수의 자료형이 해당 형태를 결정짓기 때문으로, 우리가 new로 변수에 저장하면 인스턴스의 시작주소만 알 뿐 참조 변수는 그 뒤의 인스턴스에 무엇이 있는지 모른다. 따라서 Car형으로 선언해서 car변수에 저장될 값은 Car 클래스의 멤버들만 올 수 있음을 알려주는 것이다. 
    - **변수의 크기를 결정지음**
